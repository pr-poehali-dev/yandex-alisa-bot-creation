        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          {!hasProfile && !isEditing ? (
            <div className="text-center py-4">
              ...
              <p className="text-xs text-gray-400 mb-4">Создайте профиль, чтобы сохранять переписки и добавлять друзей</p>
              <button
                onClick={() => { setEditName(""); setEditUsername(""); setEditBio(""); setIsEditing(true); }}
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs text-gray-400 mb-1 block">Юзернейм</label>
                <div className="relative">
                ...
                <p className="text-[10px] text-gray-300 mt-1">Только a-z, 0-9, _ (3–20 символов)</p>
              </div>
              <div>
              ...
              <div className="flex gap-2 pt-1">
                <button onClick={() => setIsEditing(false)} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-500 hover:bg-gray-50 transition-all">Отмена</button>
                <button onClick={saveProfile} disabled={!editName.trim()} className="flex-1 py-2.5 rounded-xl text-sm font-medium text-white transition-all disabled:opacity-40 active:scale-95"
            ...
            <div>
              ...
              <div className="flex gap-2 mt-4">
                <button
                  onClick={() => { setEditName(profile.name); setEditUsername(profile.username); setEditBio(profile.bio); setIsEditing(true); }}
                ...
                <button onClick={deleteProfile} className="flex items-center justify-center gap-2 px-3 py-2 rounded-xl border border-red-100 text-sm text-red-400 hover:bg-red-50 transition-all">
      ...
      {deleteConfirmId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background: "rgba(0,0,0,0.4)" }} onClick={() => setDeleteConfirmId(null)}>
          <div className="bg-white rounded-2xl p-6 max-w-xs w-full shadow-xl text-center" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2">
              <button onClick={() => setDeleteConfirmId(null)} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-500 hover:bg-gray-50 transition-all">Отмена</button>
              <button onClick={() => deleteChat(deleteConfirmId)} className="flex-1 py-2.5 rounded-xl bg-red-400 text-sm font-medium text-white hover:bg-red-500 transition-all">Удалить</button>
            </div>
          </div>
        </div>
      )}
